 ...
 */
function trace(str){if(typeof console != "undefined")console.log(str);}
...
function UGThumbsGeneral(){
	...
	var g_options = {
			...
			thumb_border_color:"rgba(133, 132, 116, 0.57)",				//thumb border color
			thumb_over_border_width:1,					//thumb border width in mouseover state
			thumb_over_border_color:"rgba(133, 132, 116, 0.57)",			//thumb border color in mouseover state
			thumb_selected_border_width:2,				//thumb width in selected state
			thumb_selected_border_color:"rgba(133, 132, 116, 0.57)",		//thumb border color in selected state
			thumb_round_corners_radius:0,				//thumb border radius
			thumb_color_overlay_effect:true,			//true,false - thumb color overlay effect, release the overlay on mouseover and selected states
			thumb_overlay_color:"",				//thumb overlay color
		...
		function appendHtmlThumbFromItem(itemIndex, imageEffectClass){
			...
			var thumbStyle = "style='z-index:"+zIndex+";'";
			 ...
			 if(g_temp.customThumbs){g_temp.funcSetCustomThumbHtml(objThumbWrapper, objItem);}
			 //add thumb wrapper object to items array
			 g_arrItems[itemIndex].objThumbWrapper = objThumbWrapper;return(objThumbWrapper);}
		/**
		 * append the thumbs html to some parent
		 */
		this.setHtmlThumbs = function(objParent, isAppend){g_objParent = objParent;
		...
		this.setThumbOverStyle = function(objThumb){
			...
			if(g_temp.isEffectOverlay){var isSet = (g_options.thumb_overlay_reverse == true)?true:false;setThumbColorOverlayEffect(objThumb, isSet);}
		...
		function setThumbsBorderRadius(objThumbs){
			if(g_options.thumb_round_corners_radius <= 0)
				return(false);
			//set radius:
			var objCss = {"border-radius":g_options.thumb_round_corners_radius + "px"};
			if(objThumbs){
				objThumbs.css(objCss);
				objThumbs.find(".ug-thumb-border-overlay").css(objCss);
			}else{g_objParent.find(".ug-thumb-wrapper, .ug-thumb-wrapper .ug-thumb-border-overlay").css(objCss);}}
		/**
		...
		function placeThumbImage(objThumb, objImage, objItem){
			...
			if(g_options.thumb_image_overlay_effect == false){
				...
				redrawThumbStyle(objThumb);}g_objThis.trigger(t.events.AFTERPLACEIMAGE, objThumb);}
		/**
		...
		function getImageEffectsClass(){
			...
			for(var index in arrEffects){
				...
				switch(effect){
					case "bw":imageEffectClass += " ug-bw-effect";
					break;
					case "blur":imageEffectClass += " ug-blur-effect";
					break;
					case "sepia":imageEffectClass += " ug-sepia-effect";
					break;}}
...
function UGThumbsStrip(){
	...
	function initEvents(){
		...
		if(g_temp.isNotFixedThumbs){jQuery(g_thumbs).on(g_thumbs.events.AFTERPLACEIMAGE, onThumbPlaced);}}
	/**
	 ...
	 */
	this.init = function(gallery, customOptions){initStrip(gallery, customOptions);}
	/**
...
function UGTouchThumbsControl(){
	 ...
	 * enable the control
	 */
	this.enable = function(){g_temp.isControlEnabled = true;}
	/**
	 * disable the control
	 */
	 ...
	 */
	this.isTouchActive = function(){if(g_temp.touch_active == true)return(true);
...
function UGPanelsBase(){
	...
	function placePanelAnimation(panelDest, functionOnComplete) {
		...
		case "right": // vertical
		case "left":
			var objCss = {left : panelDest + "px"};
			break;
		case "top":
		case "bottom":
			var objCss = {top : panelDest + "px"};
			break;}
...
function UGPanelHandle(){
	...
	var g_functions = new UGFunctions();
	var g_options = {
			panel_handle_align:"bottom",		//top, middle, bottom , left, right, center - close handle tip align on the handle bar according panel orientation
	...
	function checkAndFixAlign(){
		...
		switch(orientation){
			case "right":
			case "left":
				if(g_options.panel_handle_align != "bottom" && g_options.panel_handle_align != "bottom")
					g_options.panel_handle_align = "bottom";
			break;
			case "bottom":
...
function UGStripPanel() {
	...
	function validatePanelBeforeRun() {
		if (g_options.strippanel_vertical_type == false) { // horizontal
			...
			if (g_temp.panelWidth == 0) {
				throw new Error("Strip panel error: The width not set, please set width");return (false);}
		} else { // vertical validate
			if (g_temp.panelHeight == 0) {
				throw new Error("Strip panel error: The height not set, please set height");return (false);}}
		// validate orientation
		if (g_temp.orientation == null) {
			throw new Error("Wrong orientation, please set panel orientation before run");return (false);}
		return (true);}
	/**
	 * run the panel
	 */
	 ...
	 */
	function setElementsSize() {if (g_options.strippanel_vertical_type == false)setElementsSize_hor();elsesetElementsSize_vert();}
	/**
	...
	function checkSideButtons() {if (!g_objButtonNext)return (true);
		...
		if (pos <= limits.minPos)g_functions.disableButton(g_objButtonNext);
		else
	 ...
	 * init panel events
	 */
	function initEvents() {if (g_temp.isEventsInited == true)return (false);g_temp.isEventsInited = true;
		// buttons events
		if (g_objButtonNext) {
			// add hove class
...
function UGGridPanel(){
	 ...
	 * init the grid panel
	 */
	function initGridPanel(gallery, customOptions){g_gallery = gallery;validateOrientation();
...
function UGThumbsGrid(){
	 ...
	 * set the options of the strip
	 */
	function setOptions(objOptions){g_options = jQuery.extend(g_options, objOptions);g_thumbs.setOptions(objOptions);
...
function UGTiles(){
	...
	function doAfterTransition(){
		...
		}else{g_objTileDesign.triggerSizeChangeEventAllTiles();doAfterTransition_setClasses();}}
	...
	function fillTilesVars(){
		...
		} else {g_vars.colGap = g_options.tiles_space_between_cols_mobile;}
		//set gallery width
		...
		if(g_options.tiles_exact_width == true){
			...
			}else{g_vars.totalWidth = g_vars.numCols * (g_vars.colWidth + g_vars.colGap) - g_vars.colGap;}
		} else {
	 ...
	 * init col heights
	 */
	function initColHeights(){g_vars.maxColHeight = 0;
...
function UGTileDesign(){
	...
	var g_defaults = {
			...
			tile_textpanel_padding_top:18,
			tile_textpanel_padding_bottom:18};
	var g_temp = {
		ratioByHeight:0,
		ratioByWidth:0,
		eventSizeChange: "thumb_size_change",
	...
	function setHtmlThumb(objThumbWrapper, objItem){
		...
		if(g_options.tile_enable_shadow == true){
			...
			objCss["box-shadow"] = htmlShadow;}objThumbWrapper.css(objCss);
		...
		if(g_options.tile_enable_image_effect == true){
			...
			if(g_options.tile_image_effect_reverse == true){objThumbWrapper.children(".ug-tile-image-overlay").fadeTo(0,0);}}
	 ...
	 */
	function isOverStyle(objTile){if(objTile.hasClass("ug-thumb-over"))return(true);return(false);}
	...
	function positionElements_textpanel(objTile, panelType, tileWidth, tileHeight){
		...
		if(isPosition){if(g_temp.isTextpanelOutside == true && tileHeight && g_options.tile_textpanel_position == "bottom"){
	...
	function setTextpanelEffect(objTile, isActive){
		...
		if(!objTextPanel)
			return(true);
		if(g_options.tile_textpanel_appear_type == "fade"){
			...
			}else{objTextPanel.stop(true).animate(startClass, animationDuration);}
		}else{		//fade effect
			if(isActive == true){objTextPanel.stop(true).fadeTo(animationDuration, 1);
			}else{objTextPanel.stop(true).fadeTo(animationDuration, 0);}}}
	...
	function onPlaceImage(data, objTile, objImage){
		positionElements(objTile);
		objImage.fadeTo(0,1);objTile.data("image_placed", true);}
	...
	this.getTileImage = function(objTile){
		var objImage = objTile.find("img.ug-thumb-image");return(objImage);}
...
function UGAviaControl(){
	 ...
	 */
	function getMousePos(event){if(g_isVertical == false)return(event.pageX);return(event.pageY);}
...
function UGSlider(){
	...
	var g_options = {
		  ...
		  slider_image_border_color:"#6B470F",			//image border color
		  ...
		  slider_play_button_offset_hor:10,	       		 //play button horizontal offset
	...
	function setHtmlSlider(objParent){
		...
		if(g_options.slider_enable_text_panel == true){g_objTextPanel.appendHTML(g_objSlider);
	...
	function resizeSlideItem(objSlide){
		...
		if(index === undefined || index == null)
			return(false);
		var objItem = g_gallery.getItem(index);
		if(!objItem)return(false);setItemToSlide(objSlide, objItem);}
	...
	function setItemToSlide(objSlide, objItem){
		try{var objItemWrapper = objSlide.children(".ug-item-wrapper");
	 ...
	 */
	function checkMobileModify(){var isMobile = g_gallery.isMobileMode();if(isMobile)modifyForMobile();else modifyForDesctop();}
	 ...
	 * @param noAnimation
	 */
	function checkAndShowControls(noAnimation){if(g_options.slider_controls_always_on == true)showControls(noAnimation);}
	...
	function onSlideImageLoaded(data, objSlide){
		...
		var objSize = g_functions.getElementSize(objImage);
		g_functions.placeElement(objButtonVideoPlay, "bottom", "right", objSize.left, objSize.top, objImage);}
	/**
...
function UGTextPanel(){
	...
	var g_options = {
			...
			textpanel_padding_top:4,					//textpanel padding top
			textpanel_padding_bottom:4,				//textpanel padding bottom
			textpanel_height:null,						//textpanel height. if null it will be set dynamically
			textpanel_padding_title_description:0,		//the space between the title and the description
			textpanel_padding_right:11,				//cut some space for text from right
			textpanel_padding_left:11,					//cut some space for text from left
			textpanel_fade_duration:200,				//the fade duration of textpanel appear
			textpanel_enable_title:true,				//enable the title text
			textpanel_enable_description:true,			//enable the description text
			textpanel_enable_bg:false,					//enable the textpanel background
...
function UGProgressBar(){
	...
	var g_options = {
		slider_progressbar_color:"#858474",			//progress bar color
...
function UGWistiaAPI(){
	...
	this.putVideo = function(divID, videoID, width, height, isAutoplay){
		if(isWistiaActive()){putVideoActually(divID, videoID, width, height, isAutoplay);return(true);}
...
function UGSoundCloudAPI(){
	 ...
	 * check if sound cloud active
	 */
	function isSCActive(){return(typeof SC != "undefined");}
...
function UGVimeoAPI(){
	 ...
	 * init events
	 */
	function initEvents(){if(!g_player)return(false);

		//set "cuechange" event
		g_player.addEvent('cuechange', function(){if(g_cueChangeAutoplay == true)t.play();});

		//set "play" event
		g_player.addEvent('play', function(){g_objThis.trigger(t.events.START_PLAYING);});

		//set "pause event"
		g_player.addEvent('pause', function(){g_objThis.trigger(t.events.STOP_PLAYING);});
	...
	this.destroy = function(){
		try{
			if(g_player){
				...
				g_player.clearVideo();
				g_player.destroy();}
		}catch(objError){jQuery("#"+g_lastContainerID).html("");}}
...
function UGVideoPlayer(){
	 ...
	 */
	function onCloseButtonClick(){t.hide();}
	 ...
	 * hide the player
	 */
	this.hide = function(){if(t.isVisible() == false)return(true);
...
function UGLightbox(){
	...
	var g_defaultsCompact = {
			...
			lightbox_textpanel_title_top:0,
			lightbox_textpanel_desc_text_align:"center",
			lightbox_textpanel_padding_left:0,			//the padding left of the textpanel
			lightbox_textpanel_padding_right:0};
	function __________GENERAL_________(){};
	/**
	 * init the gallery
	 */
...
function UG_API(gallery){
	...
	this.reloadGallery = function(customOptions){
		...
		//restore events:
		g_arrEvents.map(function(obj){t.on(obj.event,obj.func,true);});}
